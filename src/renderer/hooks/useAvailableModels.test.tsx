...
describe('useAvailableModels', () => {
  ...
  it('loads supported models on provider change', async () => {
    vi.mocked(lmbase.providers.fetchModels).mockResolvedValueOnce({
      models: ['gpt-4o'],
      message: 'OK',
    });
    ...
    expect(lmbase.providers.fetchModels).toHaveBeenCalledWith({ provider: 'openai' });
  ...
  it('fetches live models explicitly and falls back to supported list', async () => {
    vi.mocked(lmbase.providers.fetchModels)
    ...
    expect(lmbase.providers.fetchModels).toHaveBeenLastCalledWith({ provider: 'openai' });
